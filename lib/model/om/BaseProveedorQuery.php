 ...
 *
 * @method     ProveedorQuery leftJoinRegistrocompra($relationAlias = null) Adds a LEFT JOIN clause to the query using the Registrocompra relation
 * @method     ProveedorQuery rightJoinRegistrocompra($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Registrocompra relation
 * @method     ProveedorQuery innerJoinRegistrocompra($relationAlias = null) Adds a INNER JOIN clause to the query using the Registrocompra relation
 *
 ...
 *
 * @method     Proveedor findOneByIdproveedor(int $idproveedor) Return the first Proveedor filtered by the idproveedor column
...
abstract class BaseProveedorQuery extends ModelCriteria
{
	...
	{
		$sql = 'SELECT `IDPROVEEDOR`, `NOMBRE` FROM `proveedor` WHERE `IDPROVEEDOR` = :p0';
		try {
	...
	{
		if (is_array($idproveedor) && null === $comparison) {
	...
	/**
	 * Filter the query by a related Registrocompra object
	 *
	 * @param     Registrocompra $registrocompra  the related object to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ProveedorQuery The current query, for fluid interface
	 */
	public function filterByRegistrocompra($registrocompra, $comparison = null)
	{
		if ($registrocompra instanceof Registrocompra) {
			return $this
				->addUsingAlias(ProveedorPeer::IDPROVEEDOR, $registrocompra->getIdprovee(), $comparison);
		} elseif ($registrocompra instanceof PropelCollection) {
			return $this
				->useRegistrocompraQuery()
				->filterByPrimaryKeys($registrocompra->getPrimaryKeys())
				->endUse();
		} else {
			throw new PropelException('filterByRegistrocompra() only accepts arguments of type Registrocompra or PropelCollection');
		}
	}

	/**
	 * Adds a JOIN clause to the query using the Registrocompra relation
	 *
	 ...
	 * @return    ProveedorQuery The current query, for fluid interface
	 */
	public function joinRegistrocompra($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		$tableMap = $this->getTableMap();
		$relationMap = $tableMap->getRelation('Registrocompra');
		...
		if($relationAlias) {
			...
			$this->addJoinObject($join, 'Registrocompra');
		}
	...
	/**
	 * Use the Registrocompra relation Registrocompra object
	 *
	 * @see       useQuery()
	 *
	 * @param     string $relationAlias optional alias for the relation,
	 ...
	 * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 *
	 * @return    RegistrocompraQuery A secondary query class using the current class as primary query
	 */
	public function useRegistrocompraQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		return $this
			->joinRegistrocompra($relationAlias, $joinType)
			->useQuery($relationAlias ? $relationAlias : 'Registrocompra', 'RegistrocompraQuery');
	}
