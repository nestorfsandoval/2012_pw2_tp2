...
/**
 * Base class that represents a row from the 'privilegios' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BasePrivilegios extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'PrivilegiosPeer';
	...
	/**
	 * The value for the idprivilegios field.
	 * @var        int
	 */
	protected $idprivilegios;
	...
	/**
	 * Get the [idprivilegios] column value.
	 * 
	 * @return     int
	 */
	public function getIdprivilegios()
	{
		return $this->idprivilegios;
	}
	...
	/**
	 * Set the value of [idprivilegios] column.
	 * 
	 * @param      int $v new value
	 * @return     Privilegios The current object (for fluent API support)
	 */
	public function setIdprivilegios($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->idprivilegios !== $v) {
			$this->idprivilegios = $v;
			$this->modifiedColumns[] = PrivilegiosPeer::IDPRIVILEGIOS;
		}
	...
	public function setDescripcion($v)
	{
		...
		if ($this->descripcion !== $v) {
			$this->descripcion = $v;
			$this->modifiedColumns[] = PrivilegiosPeer::DESCRIPCION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->idprivilegios = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = PrivilegiosPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Privilegios object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PrivilegiosPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = PrivilegiosQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePrivilegios:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePrivilegios:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePrivilegios:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePrivilegios:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				PrivilegiosPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(PrivilegiosPeer::IDPRIVILEGIOS)) {
			$modifiedColumns[':p' . $index++]  = '`IDPRIVILEGIOS`';
		}
		if ($this->isColumnModified(PrivilegiosPeer::DESCRIPCION)) {
			$modifiedColumns[':p' . $index++]  = '`DESCRIPCION`';
		}

		$sql = sprintf(
			'INSERT INTO `privilegios` (%s) VALUES (%s)',
		...
		try {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`IDPRIVILEGIOS`':
						$stmt->bindValue($identifier, $this->idprivilegios, PDO::PARAM_INT);
						break;
					case '`DESCRIPCION`':
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PrivilegiosPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PrivilegiosPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Privilegios'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Privilegios'][$this->getPrimaryKey()] = true;
		$keys = PrivilegiosPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getIdprivilegios(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdprivilegios($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PrivilegiosPeer::IDPRIVILEGIOS)) $criteria->add(PrivilegiosPeer::IDPRIVILEGIOS, $this->idprivilegios);
		if ($this->isColumnModified(PrivilegiosPeer::DESCRIPCION)) $criteria->add(PrivilegiosPeer::DESCRIPCION, $this->descripcion);
	...
	{
		$criteria = new Criteria(PrivilegiosPeer::DATABASE_NAME);
		$criteria->add(PrivilegiosPeer::IDPRIVILEGIOS, $this->idprivilegios);
	...
	/**
	 * Generic method to set the primary key (idprivilegios column).
	 *
	...
	{
		return null === $this->getIdprivilegios();
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Privilegios (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setIdprivilegios(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Privilegios Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PrivilegiosPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PrivilegiosPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this Privilegios is new, it will return
	...
	{
		$this->empleadosScheduledForDeletion = $this->getEmpleados(new Criteria(), $con)->diff($empleados);

		foreach ($empleados as $empleado) {
			// Fix issue with collection modified by reference
			if ($empleado->isNew()) {
				$empleado->setPrivilegios($this);
			}
	...
	public function addEmpleado(Empleado $l)
	{
		...
		if (!$this->collEmpleados->contains($l)) { // only add it if the **same** object is not already associated
	 ...
	 * @return     PropelCollection|array Empleado[] List of Empleado objects
	 */
	public function getEmpleadosJoinCiudadRelatedByIdCiudad($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = EmpleadoQuery::create(null, $criteria);
		$query->joinWith('CiudadRelatedByIdCiudad', $join_behavior);
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BasePrivilegios:' . $name))
		{
