...
/**
 * Base class that represents a row from the 'artista' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseArtista extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'ArtistaPeer';
	 ...
	 * @return     Artista The current object (for fluent API support)
	 */
	public function setIdartista($v)
	{
		...
		if ($this->idartista !== $v) {
			$this->idartista = $v;
			$this->modifiedColumns[] = ArtistaPeer::IDARTISTA;
		}
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = ArtistaPeer::NOMBRE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->idartista = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = ArtistaPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Artista object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ArtistaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ArtistaQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseArtista:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseArtista:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseArtista:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseArtista:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				ArtistaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ArtistaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ArtistaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Artista'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Artista'][$this->getPrimaryKey()] = true;
		$keys = ArtistaPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getIdartista(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdartista($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ArtistaPeer::IDARTISTA)) $criteria->add(ArtistaPeer::IDARTISTA, $this->idartista);
		if ($this->isColumnModified(ArtistaPeer::NOMBRE)) $criteria->add(ArtistaPeer::NOMBRE, $this->nombre);
	...
	{
		$criteria = new Criteria(ArtistaPeer::DATABASE_NAME);
		$criteria->add(ArtistaPeer::IDARTISTA, $this->idartista);
	...
	/**
	 * Generic method to set the primary key (idartista column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Artista (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setIdartista(NULL); // this is a auto-increment column, so set to default value
		}
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Artista Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Artista is new, it will return
	 ...
	 * Otherwise if this Artista is new, it will return
	 * an empty collection; or if this Artista has previously
	 * been saved, it will retrieve related Productos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Artista.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array Producto[] List of Producto objects
	 */
	public function getProductosJoinGenero($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ProductoQuery::create(null, $criteria);
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseArtista:' . $name))
		{
