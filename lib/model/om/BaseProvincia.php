...
abstract class BaseProvincia extends BaseObject  implements Persistent
{
	...
	/**
	 * Get the [idprovincia] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [idprovincia] column.
	 * 
	...
	public function setIdprovincia($v)
	{
		...
		if ($this->idprovincia !== $v) {
			$this->idprovincia = $v;
			$this->modifiedColumns[] = ProvinciaPeer::IDPROVINCIA;
		}
	...
	public function setProvincia($v)
	{
		...
		if ($this->provincia !== $v) {
			$this->provincia = $v;
			$this->modifiedColumns[] = ProvinciaPeer::PROVINCIA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->idprovincia = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->provincia = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$this->modifiedColumns[] = ProvinciaPeer::IDPROVINCIA;
		if (null !== $this->idprovincia) {
			throw new PropelException('Cannot insert a value for auto-increment primary key (' . ProvinciaPeer::IDPROVINCIA . ')');
		}

		 // check the columns in natural order for more readable SQL queries
		if ($this->isColumnModified(ProvinciaPeer::IDPROVINCIA)) {
			$modifiedColumns[':p' . $index++]  = '`IDPROVINCIA`';
		}
		if ($this->isColumnModified(ProvinciaPeer::PROVINCIA)) {
		...
		try {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`IDPROVINCIA`':
						$stmt->bindValue($identifier, $this->idprovincia, PDO::PARAM_INT);
						break;
					case '`PROVINCIA`':
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setProvincia($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProvinciaPeer::IDPROVINCIA)) $criteria->add(ProvinciaPeer::IDPROVINCIA, $this->idprovincia);
		if ($this->isColumnModified(ProvinciaPeer::PROVINCIA)) $criteria->add(ProvinciaPeer::PROVINCIA, $this->provincia);
	...
	{
		$criteria = new Criteria(ProvinciaPeer::DATABASE_NAME);
		$criteria->add(ProvinciaPeer::IDPROVINCIA, $this->idprovincia);
	...
	/**
	 * Generic method to set the primary key (idprovincia column).
	 *
	 ...
	 * @return     PropelCollection|array Ciudad[] List of Ciudad objects
	 * @throws     PropelException
	 */
	public function getCiudads($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collCiudads || null !== $criteria) {
	...
	{
		$this->ciudadsScheduledForDeletion = $this->getCiudads(new Criteria(), $con)->diff($ciudads);
	...
	public function addCiudad(Ciudad $l)
	{
		...
		if (!$this->collCiudads->contains($l)) { // only add it if the **same** object is not already associated
			$this->doAddCiudad($l);
		}
