...
<form action="<?php echo url_for('catalogo/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?idproducto='.$form->getObject()->getIdproducto() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<a href="<?php echo url_for('catalogo/index') ?>">Back to list</a>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Delete', 'catalogo/delete?idproducto='.$form->getObject()->getIdproducto(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
